...
ax.set_xlabel("UMAP1")
ax.set_ylabel("UMAP2")
ax.legend(markerscale=5, frameon=False, fontsize=12, title="Tissue", loc="upper left")  # ★ 右图左上角空白区，不遮任何点
ax.set_aspect("equal")
...
H5AD_MAIN    = f"{BASE}/GSE182434/adata_processed.h5ad"
...
# ★ 用户要求：去掉左侧 Normal/Malignant 分组文字标签（set_yticks 连刻度一起清，避免 FixedLocator 匹配问题）
for _ax in fig.axes:
    _lbls = [_t.get_text() for _t in _ax.get_yticklabels()]
    if any(('Tonsil' in _s) or ('Malignant' in _s) or (_s == 'Normal') for _s in _lbls):
        _ax.set_yticks([])
...
# 1. 读取主 AnnData
# ----------------------------
adata = sc.read_h5ad(f"{BASE_DIR}/adata_processed.h5ad")
